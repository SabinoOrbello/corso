...
const results = [{},{},{},{},{},{},{},{},{},{}]; // inizializzazione dell'array contenente le domande e le risposte esatte

let punteggio = 0; // variabile globale del punteggio
let index= 0; // indice della domanda
const difficolta = parseInt(localStorage.getItem('storageName'))  // prendo la variabile difficoltà passata dai radioButton della pagina precedente
questions.length = difficolta;   // impostiamo la lunghezza dell'array in base al value della difficoltà selezionata
...
const timer = function () {
  // abbiamo diviso timer in due div per creare l'effetto visivo del tempo che diminuisce
    ...
    let gradiTimer = 360 -((i * 360) /60)  // forma per definire a quanti gradi delo cerchio corrisponde lo scorrere del tempo
    ...
    timerPar.innerHTML = `<p style = "font-size : 25px">Seconds</p><p> ${i}</p><p>reamining</p>`;

    // allo scadere del tempo fa partire la funzione che cambia la domanda
    if (i<=0) {
        premiTasti();
    }
    i--;
...
//  funzione che genera le domande e le risposte, prende come parametri un array e un indice (inizializzato ad inizio pagina)
...
const generaDomanda = function (arr, indice) {
    ...
    results[index].domanda = arr[index].question; // inseriamo la domanda nell'array risultati ogni volta che viene generata una nuova domana
    results[index].risposta_esatta = arr[index].correct_answer; //

    
    const domanda = document.getElementById('domanda');  // selezioniamo l'elemento con id domanda
    risposte = document.getElementsByClassName('bottoneRisposta');  // selelzioniamo gli elementi con classe bottoneRisposta creati in precedenza
    domanda.innerHTML = arr[indice].question;   // inseriamo la domanda nell'h2


      // se le risposte sbagliate sono 3 la risposta corretta potrà andare in modo randoico in uno dei 4 bottoni
    ...
    risposte[rispIndex].classList.remove('sbagliata')  // rimuove la classe risposta sbagliata perchè la risposta inserita è quella giusta
    risposte[rispIndex].classList.add('corretta')   // aggiunge la classe corretta


     // assegnazione risposte sbagliate - in base al valore di rispIndex(risposta giusta) assume popoliamo gli altri bottoni
    switch (rispIndex) {
        case 0: 
...
}  else {  // se invece la risposta sbagliata è solo una, faremo lo stesso procedimento ma con solo due bottoni
...
generaDomanda(questions, index);   // chiamiamo la funzione per popolare la pagina quado viene caricata


  // lancia le funzioni necessarie al momento del click su una delle risposte
const premiTasti = function () {
setTimeout(function(){   // delay di 0.5 sec per mostarare un feedback sulla risposta (giusta,sbagliata)
    ...
    i=59;
    interval1;
  } else{  // se le domande sono terminate elimina il timer e genera la terza pagina(con la funzione generaRisultati)
...
// funzione che aggiunge l'event listener ad ogni bottobe al click fa partire la funzione premiTasti
...
const segnaPunti = function () {
  ...
  corretto.addEventListener('click', function () {
    punteggio ++;
    corretto.setAttribute('style', 'background-color: green') // bottone che se premuto cambia il suo colore di background in verde
...
// funzione che cambia il numero della domanda mostrata a fine pagina
...
// funziona che genera la pagina dei risultati in base al risultato ottenuto
const generaRisultati = function () {
  ...
  contenitore.setAttribute('style', 'margin-top : 2em;')  // diamo un pò di margine al contatore principale

  const titolo = document.getElementById('domanda'); 
  titolo.innerText = 'Results';   // aggiungiamo il titolo a pagina 3

  const sottoTitolo = document.getElementById('sottotitolo')  
  sottoTitolo.innerText = 'The summary of your answers:'  //aggiunta del sottotitolo a pag 3

  const corpoCentrale = document.querySelector('.risposte');
  corpoCentrale.classList.add('centraleFlex')  // aggiungiamo la classe centraleFlex per dividere il contenuto in 3 parti

  // prima parte corpo centrale (risposte giuste)
  const div1 = document.createElement('div')
  corpoCentrale.appendChild(div1);
  const corrette = document.createElement('h2');
  div1.appendChild(corrette);

  corrette.innerHTML = 'Correct' +'<br>' +'<strong>' + Math.round((punteggio/questions.length)*100) + '</strong>' +'%' // calcolo della percentuale risposte giuste dato il punteggio
  const par1 = document.createElement('p');
  par1.innerHTML = punteggio + '/' + questions.length + ' questions';
  div1.appendChild(par1);

  // seconda parte del corpo centrale con un indicatore grafico per risposte giuste e sbagliate (cerchio)
  const div2 = document.createElement('div');     // cerchio più grande che diventa celeste in base alle risp corrette e rosa in base alle risp sbagliate
  ...
  const parPiccolo =document.createElement('b'); 
  ...
  if(punteggio >= pass){  // popolerà i paragrafi in un modo se il punteggio è maggiore del 60%
    ...
    parPiccolo.innerHTML = "Well send you the certificate in few minutes. check your email (including promotion / spam folder"
  ...
  // terza parte corpo centrale (risposte sbagliate)
  const div3 = document.createElement('div');
  const sbagliate = document.createElement ('h2')
  let risposteSbagliate = questions.length - punteggio;
  sbagliate.innerHTML = 'Wrong' + '<br>' + '<strong>' + Math.round ((risposteSbagliate/questions.length)* 100) +'</strong>' + '%';
  corpoCentrale.appendChild(div3);
  div3.appendChild(sbagliate);
  const par3 = document.createElement('p')
  par3.innerHTML = risposteSbagliate + '/' +questions.length + 'questions';
...
// funzione che crea la barra dei progressi sulle domande rimanenti
function updateProgressBar() { 
  totalQuestions = questions.length;
  const progress = (index / (totalQuestions +1)) * 100;  // calola la percentuale delle risposte date
